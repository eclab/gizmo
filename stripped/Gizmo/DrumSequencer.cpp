...
// 3. GROUP TRACK DATA[groups][tracks] -- this is one half byte per group per track  Thus 2 pairs are stuffed into a byte.  The total number of pairs per track is HALFTRACKS.
...
#define GET_NOTE_OFFSET(group, track)                           (NOTE_OFFSET + (local.drumSequencer.numTracks * (uint16_t) local.drumSequencer.numNotes) * group + local.drumSequencer.numNotes * (uint16_t) track)
...
uint8_t getActualGroupLength(uint8_t group)
    ...
    if (gl == DRUM_SEQUENCER_GROUP_LENGTH_DEFAULT) return nn;
...
uint8_t numFormatNotes(uint8_t format)
    {
    const uint8_t notes[DRUM_SEQUENCER_NUM_FORMATS] = { 8, 8, 16, 16, 32, 32, 64, 64 };
    return notes[format];
    } 
...
uint8_t numFormatTracks(uint8_t format)
    {
    const uint8_t tracks[DRUM_SEQUENCER_NUM_FORMATS] = { 16, 12, 16, 12, 16, 12, 16, 12 };
...
uint8_t numFormatGroups(uint8_t format)
    ...
    const uint8_t groups[DRUM_SEQUENCER_NUM_FORMATS] = { 13, 15, 8, 11, 4, 6, 2, 3 };
...
void initDrumSequencer(uint8_t format)
    ...
    // group length 0 is DRUM_SEQUENCER_GROUP_LENGTH_DEFAULT
    // note speed 0 is DRUM_SEQUENCER_NOTE_SPEED_DEFAULT
    memset(data.slot.data.drumSequencer.data, 0, DRUM_SEQUENCER_DATA_LENGTH);
        
    // MIDI channel 17 is DRUM_SEQUENCER_MIDI_OUT_DEFAULT
    // note velocity should be DRUM_SEQUENCER_MAX_NOTE_VELOCITY
    // note pitch is set to DRUM_SEQUENCER_INITIAL_NOTE_PITCH
    // mute is 0
    // pattern OOOO is 15 for all group/track combos
    for(uint8_t i = 0; i < numFormatTracks(format); i++)
        {
        setMIDIChannel(i, DRUM_SEQUENCER_MIDI_OUT_DEFAULT);
        setNoteVelocity(i, DRUM_SEQUENCER_MAX_NOTE_VELOCITY);
        setNotePitch(i, DRUM_SEQUENCER_INITIAL_NOTE_PITCH);
        for(uint8_t j = 0; j < numFormatGroups(format); j++)
            {
    ...
    for(uint8_t i = 0; i < DRUM_SEQUENCER_NUM_TRANSITIONS; i++)
        {
        if (i < MAX_DRUM_SEQUENCER_GROUPS)
            {
...
void unpackDrumSequenceData()
    ...
    memcpy(data.slot.data.drumSequencer.data, data.slot.data.drumSequencer.data, DRUM_SEQUENCER_DATA_LENGTH);
    for(uint8_t i = 0; i < DRUM_SEQUENCER_NUM_TRANSITIONS; i++)
        {
    ...
    if (local.drumSequencer.transitionCountdown == 0)
        ...
        if (local.drumSequencer.currentTransition > DRUM_SEQUENCER_NUM_TRANSITIONS ||
            ...
            if (local.drumSequencer.currentTransition == 0)
                {
                repeat = DRUM_SEQUENCER_TRANSITION_REPEAT_LOOP;
                group = 0;
                }
            else
                {
        ...
        case MENU_SELECTED:
            ...
            switch(currentDisplay)
                ...
                case DRUM_SEQUENCER_MENU_RESET:
                    {
                    /// FIXME: Not sure what we should do here if anything
...
void stateDrumSequencerPitch()
    ...
    playDrumSequencer();                //// FIXME:   Should we do this?
    }
...
void playDrumSequencer()
    ...
    if (notePulse && (local.drumSequencer.playState == PLAY_STATE_PLAYING))
        ...
        if (local.drumSequencer.currentPlayPosition == 0)
            {
            uint8_t noteSpeedType = getNoteSpeed(local.drumSequencer.currentGroup);
            if (noteSpeedType == 0) 
                {
...
/// Resets the drum sequencer to its start position.  Not sure if this is useful.
...
///// INCOMING MIDI DATA
    ...
    /*
      ...
      if (local.drumSequencer.performanceMode)
      {
      drumSequencerAdvanceMute(itemNumber - CC_EXTRA_PARAMETER_A);
      }
      else
      {
...
////////  BULK EDITING (COPY-PASTE, ETC.)

//// At present this code is commented out.  It'd be a bit of work to get it up and running.
