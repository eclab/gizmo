...
{\large \bf An Arduino MIDI Tool, Version 10\\
...
Due to space restrictions, choking is done in an odd way.  To set up track B to choke track A, you must first arrange for track B to be {\it immediately below} track A.  Then set the MIDI Channel of track B to \texttt{CHOKE}.  Track B will use the same MIDI Channel as A, and will choke it.  If you want multiple tracks (such as B and C), set B directly below A, and set C directly below B.  Then set the MIDI Channel of {\it both} B and C to  \texttt{CHOKE}.  Muting or (via Patterns) hushing Track B disables its choking, so to set up track B to {\it only} choke A, but play no notes itself, set Track B's drum note to something out of the drum synth's range.  Finally, if you'd like a track B to choke {\it itself} prior to playing a note, set it below some other track A with the same MIDI Channel, and then set B's MIDI Channel to  \texttt{SELF CHOKE}.  Track B will use the same MIDI Channel as A, but will choke itself.\footnote{Yes, I know this is weird\,---\,it's the only way I could fit it in with memory restrictions.} 
