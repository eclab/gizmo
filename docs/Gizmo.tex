...
{\large \bf An Arduino MIDI Tool, Version 5\\
...
\paragraph{Performance Mode} [Mega only]  If you simultaneously Long-Press the Select and Middle buttons when in either Edit or Play Position Mode, you'll enter Performance Mode.  This mode is meant to support performing with completed sequences.  Here, if you play notes they're not entered as data in the Sequencer.  Instead one of two things will happen depending on how you've configured things: either the notes will be routed to some channel in MIDI Out, or the notes will be used to {\it transpose} the sequence as it's playing.  A track is transposed only if you have turned its ``Transpose'' option on (see Menu Options below).
...
\item {\bf Fader (Track)}\quad This lets you specify a volume modifier which is multiplied against the velocities of the notes in the track.  Possible values are 0...31, where 0 completely hushes the notes, 16 lets them play at their specified velocities, and 31 is roughly twice their velocity.  In general, values larger than or smaller than 16 will make the notes louder or quieter respectively.
...
\item {\bf Pattern (Track)} (Mega only)\quad Lets you specify the four-iteration pattern for this track.  An {\tt O} indicates that the track will be played, and a {\tt -} indicates that it will be hushed.  Your options are: \texttt{OOOO, O-O-, -O-O, O---, -O--, --O-, ---O, OO--, -OO-, --OO, O--O,  OOO-, O-OO, OO-O,} or {\tt RND}, indicating that each iteration the track is hushed at random.
\item {\bf Edit} (Mega only)\quad A submenu for cut/copy/paste-like operations:
\begin{itemize}
\item {\bf Mark} (Mega only)\quad Records the current track and edit position as the {\it mark position}.
\item {\bf Copy} (Mega only)\quad Copies the data at the mark position to the current track starting at the current edit position.  The copied data is wrapped around when it goes beyond the end of the current track.
\item {\bf Splat} (Mega only)\quad Copies the data at the mark position to the current track starting at the current edit position.  The copied data is {\it not} wrapped around when it goes beyond the end of the current track: it just stops there.
\item {\bf Move} (Mega only)\quad Works like Copy, except that the mark position track is then erased.
\end{itemize}
...
\begin{itemize}
\item {\bf Play Along}  This menu option lets you choose to {\it play along} with the sequence (as you play notes they will be sent to MIDI Out)  or to {\it transpose} the sequence in real-time by pressing a key.   The menu options are {\it None} (meaning, confusingly, to send out the default MIDI Out Channel), 1...16 (the MIDI channel to route your played notes) or {\tt TRAN}, meaning``Transpose''.
\item {\bf Repeat Sequence} This menu option specifies how many iterations the sequence should play before it stops.  The default is ``Forever'', but you also have 1, 2, 3, 4, 5, 6, 8, 9, 12, 16, 18, 24, 32, 64, or 128 iterations as options.
\item {\bf Next Sequence} This menu option specifies {\it which} sequence the sequencer should switch to when it has completed the given sequence.  Your options are None, or 0...9.
\end{itemize} 
...
\noindent {\small
\begin{tabular}{@{}rlll@{}}
\it Control					& \it Edit Cursor Mode	& \it Play Cursor Mode	& \it Performance Mode\\
\hline\\[-0.5em]
...
Keystrokes, CC, etc.			& Enter at Edit Cursor&Enter at Play Cursor & Transpose {\it or} Route Out\\
\end{tabular}
}
...
\noindent {\small
\begin{tabular}{@{}rll@{}}
\it CC Number & CC Value	& \it Operation\\
\hline\\[-0.5em]
