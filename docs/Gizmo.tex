...
You can in fact use for 8x8 backpacks as a square, but I would not recommend it: too many calls to I2C.   {\color{red}\bf Warning:} LED screens draw significant current. Doubling the screens will potentially double the current draw on the Arduino's regulator.  You may want to run the screens at a lower brightness. I've not had any problems (and the regulator does not get warm) running for long periods of time with my customary brightness of 4 on a Mega.
