...
\item {\bf Triggered}\quad This is like Gated, except that the envelope continues even if you have release all the keys.  You restart the envelope by releasing all the keys and then pressing a new key.  This might be useful for sounds with long releases.
...
\paragraph{Intervals} A stage time interval can any \(n\) from 0...127 (or {\bf off} as discussed earlier).  The envelope can be {\bf synced} to the MIDI clock (or {\bf unsynced}). When synced, a value of \(n\) is a 32nd note, that is, \(n/8\) of a beat.  So if you want your interval to last a 4-beat measure, set \(n=32\).
