...
&\multicolumn{3}{c}{\it Testing}&\\[0.1em]
\cline{2-4}\\[-0.9em]
{\it Item} & {\it Coded} & {\it Cursory} & {\it Significant} & {\it Known Bugs}\\[0.1em]
		\hline\\[-1.5em]
Arpeggiator& $\checkmark$ & $\checkmark$ & $\checkmark$  & Fencepost error in GUI.  Scrolling through edited arpeggios will cause each note to play one too early, causing confusion.  Need to look at arpeggiation editing GUI again.\\ 
Step Sequencer& $\checkmark$ & $\checkmark$ & & Very occasionally the beat doesn't match up with the play cursor.  Possibly track velocity doesn't do much?\\ 
...
Controller& $\checkmark$ & $\checkmark$ & & Controller isn't generating proper NRPN messages, I'm pretty sure. CC works fine.\\ 
...
Keyboard Splitting& $\checkmark$ & & & Entirely untested\\
Control Voltage& $\checkmark$ & & & Entirely untested\\
\end{tabular}
}
\end{center}
