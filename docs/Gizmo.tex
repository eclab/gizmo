...
\item {\bf  \textit{Mega Only} Control Voltage.}\quad You can configure Gizmo to optionally output control voltage (CV) values from 0--5V, through two DACs, in response to notes being played.    One CV output is in response to the note pitch, and the other is in response to the note velocity.
...
\item {\bf Random}.  Chord notes are played at random.  Gizmo tries not to play the same not twice in a row.
...
Be prepared to have to replace buttons after a few months of heavy use.  My select button has begun to fail: if I press it the right way it won't indicate that it's been pressed.  Or perhaps you might get different buttons (they're 12mm pushbuttons\,---\,I redid my board with low-profile buttons which I think will be better).
