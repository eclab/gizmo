...
In certain unusual cases, you may wish the arpeggiator to send clock signals to synchronize other synthesizers with your playing.  You can manually {\bf start or stop} the clock during arpeggiation by Long-Pressing the Middle button.  If you start the clock like this, the Arpeggiator remembers this and starts the clock automatically next time you enter the arpeggiator.  If you stop the clock, then next time the Arpeggiator won't do anything when you enter the arpeggiator.  


\begin{wrapfigure}{r}{2.7in}
\begin{center}
\vspace{-1em}\includegraphics[width=1in]{updown}
\vspace{-1em}
...
There are seven preset arpeggio patterns available to you: {\it up}, {\it down}, {\it up-down}, {\it  \(+\) up-down}, {\it random}, {\it chord repeat}, and {\it assign}.  You can also create a new arpeggio pattern (discussed later) with {\it create}.  The presets work as follows:
...
The Drum Sequencer also responds to the {\it Transpose} option (because all applications do) but this is a nuisance as it changes the drum notes.  I strongly suggest you set Transpose to 0 in Options when using the Drum Sequencer.
