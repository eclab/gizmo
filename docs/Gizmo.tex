...
{\it [Mega Only]}\qquad The Splitter allows lets you split and layer your keyboard into three zones, each controlling different combinations of synthesizers.  To do this, you will specify up to two notes, {\bf Split Note A} and {\bf Split Note B}.  Split Note B is optional.  You will also specify an {\bf Alternative MIDI Out Channel}, and whether control messages should be sent to the {\bf right} (top) or {\bf left} (bottom) of the keyboard split.
...
\paragraph{Where Do Control Change, NRPN/RPN, Program Change, Channel Aftertouch, and Pitch Bend Go?}

Long-pressing the Middle Button toggles between routing them to the left of the keyboard or to the right of the keyboard.  Two LEDs will light indicating which is the current state (see Figure \ref{splitter}.

If you specified both Split Note A and Split Note B, and thus have an overlapping region, then Pitch Bend will be routed to {\it both} channels.

Note that unlike Channel Aftertouch, Polyphonic Aftertouch is routed to the channel handling the note in question.
...
Interface Utilities &  $\checkmark$ & $\checkmark$ &  $\checkmark$ & Buttons still need work: long-presses occur prematurely, and we still have some switch bouncing.\\
Storage Facility &  $\checkmark$ & $\checkmark$ &  $\checkmark$ &\\
MIDI Clock Control& $\checkmark$ & $\checkmark$ & &\\
Keyboard Splitting& $\checkmark$ & $\checkmark$  & &\\
