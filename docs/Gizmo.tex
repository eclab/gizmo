...
\item {\bf \textit{Mega Only}\quad Midi Thru: Distributing and Layering Notes, and Chord Memory.}\quad Gizmo can {\it distribute} notes to different MIDI channels as you play: this lets you play multiple mono synths as if they were polyphonic, for example.  Gizmo can also {\it layer} notes: when you play a note, Gizmo will send some \(N>1\) note-on messages instead of one.  This lets you use a polyphonic synthesizer to play a note with more than one voice, making it fatter, but not going all the way to unison.  Finally, Gizmo can perform {\it chord memory}, playing a transposed chord of your choosing starting at a note you are currently playing.
And yes, you can do all three simultaneously.
...
\begin{wrapfigure}{r}{1.8in}
\vspace{-46em}\includegraphics[width=1.8in]{octave.pdf}
...
\end{wrapfigure}
	...
	The measure counter is basically a stopwatch which counts elapsed beats, measures (bars), and phrases (up to 127 phrases).  Alternatively if can count eighths of a second,\footnote{\label{footnotetenths} Why not tenths of a second?  Because \(10 \times 60 \times 128\) is larger than \(2^{16}\), so it won't fit in a 16-bit integer.} seconds, and minutes (up to 127 minutes) via the internal clock or external MIDI clock.  If the minutes or phrases exceed 127, then \texttt{HI} is displayed in that space instead.\footnote{The Measure Counter was added to Gizmo at the request of \texttt{Inkog}, a member of http:/\!/gearslutz.com and early adopter of Gizmo.}
	
	By default, Measure counts beats, measures (bars), and phrases.  Long-press the Select button to bring up the Menu.  Here you can choose the {\bf Beats Per Bar} (a value from 1...16)\footnote{Why not 32 like Bars Per Phrase?  Because \(32 \times 32 \times 128 > 2^{16}\).  So basically for the same reason as Footnote \ref{footnotetenths}.} and the {\bf Bars per Phrase} (a value from 1...32), and as usual, call up the {\bf Options Menu} to change the {\bf Tempo}, etc.
...
\vbox{
		\begin{description}
			...
			Divide\>Use an external MIDI Clock.  Don't pass it through, but \\
			\>~~~~~~instead send out a MIDI clock pulse every \(N\) pulses received.\\
...
The {\bf Divide} option is special: instead of sending MIDI clock command every pulse, it sends MIDI clock every \(N\) pulses, thus slowing down (dividing) the clock signal.  When you choose {\bf Divide}, you'll be further asked to specify what \(N\) is:

			\begin{description}
					\item{\bf \textit{Choose}}: 2 ... 64 (the value for \(N\)) when using {\bf Divide}
			\end{description}
...
Gimzo uses nearly all available space on the Arduino Uno, and whenever I need to fix a bug I have to go on a hunting expedition to squeeze out a little more space somewhere.   You don't want to do that.  If you want to do any serious development, you should get an Arduino Mega instead, which has lots of available space.  But if you {\it insist} on extending Gizmo on the Uno with some tiny application, you can remove the Note Recorder to make space.  In \texttt{All.h} just uncomment the line that says \texttt{NO\_RECORDER} and you'll get about 1.5K of code space.  It's not a lot but should be enough to write a small app. Note that ``Recorder'' will still appear in the Menu: it just won't do anything.  To get rid of that you need to modify the state machine a bit.  I'd not bother.
