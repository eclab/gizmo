...
\item {\bf Control Change (CC)}\quad If the CC parameter is for 7-bit values, then the value is shown first, then text is scrolled with the value, then ``CC'', then the parameter.  If the CC parameter is for 14-bit values, the MSB of the value is shown first, then text is scrolled with the value, then ``CC'', then the parameter, then in parentheses the 14-bit (MSB+LSB) value.
...
\begin{itemize}
\item {\bf Channel Mode}\quad The value is shown first, then text is scrolled with the value, then ``CHANNEL MODE'', then the parameter.  Channel Mode parameters are:\footnote{Why no text to describe these things?  Because the Arduino Uno is out of space.  And they're not that common.}
...
I might add more unusual filters later.
	...
	\item{\bf Go}\\
		When you have set up your extra notes and note distribution (see below), if you choose {\bf Go}, the Thru filter starts and you can start playing.  You'll see the word ``PLAY'' on the LED.
...
{\it Item} & {\it Coded} & {\it Cursory} & {\it Significant} & {\it Known Bugs and Issues}\\[0.1em]
		\hline\\[-1.5em]
Arpeggiator& $\checkmark$ & $\checkmark$ & $\checkmark$  & Recent changes in Arpeggiator need to be tested. \\
