...
\item {\bf \textit{Mega:}\quad Keyboard Splitter.}\quad You can split incoming MIDI notes by pitch and route them to different MIDI out channels.  You can also fade notes between two channels depending on note velocity.

\item {\bf \textit{Mega}\quad MIDI Thru}\quad Gizmo can do magic with incoming notes.  It can {\it merge} and {\it block} channels%, and can map CC to NRPN
...
\item {\bf \textit{Mega:}\quad Measure Counter.}\quad You can count elapsed beats, measures (bars), and phrases; and alternatively count eighths of a second, seconds, and minutes.  The counter also responds to external MIDI clock directives.

\item {\bf \textit{Mega:}\quad Synth MIDI Helpers.}\quad Utilities designed to improve the MIDI of specific synthesizers.  At present Gizmo comes with helpers for the Waldorf Blofeld, Korg Microsampler, Oberheim Matrix 1000 (firmware 1.2) and Yamaha TX81Z.
...
\item {\bf \textit{Mega}\quad Transposition and Volume Control.}\quad You can transpose all of Gizmo's MIDI output pitch by anywhere from \(-60...60\) steps.  Additionally you can multiply the MIDI output velocity (volume) by multiples of two ranging from \(1/8\) to \(8\).

\item {\bf \textit{Mega}\quad Menu Delay.}\quad Many of Gizmo's menus display the first few letters of a menu item, then pause for some interval of time before they start scrolling the full item.  You can change this interval.

\item {\bf \textit{Mega}\quad Sysex Dumps.}\quad Arpeggios, Step Sequences, and Recordings can be uploaded and downloaded to your computer.
...
(Mega) The arpeggiator also has a {\bf performance mode} which you toggle by Long-Pressing the Middle and Select buttons simultaneously.  In this mode, if you press notes on the keyboard, they are not sent to the arpeggiator, but instead are used for one of two things:
	\begin{itemize}
...
(Mega) In some situations, you may need to manually {\bf start or stop} the clock during arpeggiation.  You can do this by Long-Pressing the Middle button.
...
90	&	Any	 & 	 	& Clear Latch {\it (Mega only)}\\
...
The Step Sequencer makes it easy to create multitrack loops common on devices like drum machines.  On the Uno, you can have twelve 16-note tracks, eight 24-note tracks, or six 32-note tracks.  On the Mega you can additionally have four 48-note tracks and three 64-note tracks.
...
\begin{itemize}
\item If current track is {\it not} muted: {\bf Schedule Mute For One Bat} \(\rightarrow\)  {\bf Schedule Mute} \(\rightarrow\) {\bf Undo}
...
\item {\bf Repeat Sequence}\quad This menu option specifies how many bars the sequence should play before it stops.  The default is ``Forever'', but you also have 1, 2, 3, 4, 5, 6, 8, 9, 12, 16, 18, 24, 32, 64, or 128 bars as options.
...
Right Knob				& Change Note Position & Leave Play Cursor Mode & Change Tempo\\
						& {\it or} Enter Play Cursor Mode && {\it (Press Back/Select when Done)}\\
...
The Controller allows you to assign to each of two buttons and four potentiometers any NRPN, RPN, Control Change (CC), or Program Change (PC) value; or (on the Mega) Pitch Bend or Channel Aftertouch.%, or control voltage.  
...
%Optional control voltage is sent out one of two Digital-to-Analog Converters or DACs you may have attached, which it refers to as DAC A (I2C address 0x62) or DAC B (I2C address 0x63).\footnote{Gizmo assumes you're DACs of this kind: https:/\!/www.adafruit.com/product/935\quad These DACs can be connected via I2C, send 0--5V, and can be set to either I2C address 0x62 or 0x63.  If you need more than 5V, you'll need to wire up your own op-amp.}
	...
	In each, you are first given the option of what kind of {\bf control type} (OFF, CC, NRPN, RPN, PC, or (on the Mega) Pitch Bend or Aftertouch)% A Voltage, or B Voltage) 
...
you would like to manipulate with the middle button.  If OFF, then pressing the button will not do anything.
	...
	\item{\textit{Mega]}\qquad }{\bf Analog Port 2} and {\bf Analog Port 3}\quad (\texttt{A2} and \texttt{A3})
...
\textit{[Mega]}\qquad The Controller also sports an {\bf 8-stage envelope} to control any single control type (CC, NRPN, RPN, PC, Pitch Bend, Channel Aftertouch)%, Voltage)
...
\textit{[Mega]}\qquad The {\bf Thru Facility} passes MIDI through the box, but if MIDI data comes in via certain channels, it applies various optional transformations to it before it sends it out:
...
%	\end{tabular}\end{center}
	...
	\item{\bf \textit{Mega:}\quad Transpose}\\
	...
	\item{\bf \textit{Mega:}\quad Volume}\\
...
\hspace{\fill}\smash{\\\includegraphics[width=1in]{note.pdf}}\vspace{-1.5em}
	...
	\item{\bf \textit{Mega:}\quad Menu Delay}\\
	...
	\item{\bf \textit{Mega:}\quad Auto Return}\\
