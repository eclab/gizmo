...
{\large \bf An Arduino MIDI Tool, Version 11\\
...
Gizmo comes built-in with a number of applications and other capabilities.  All of these capabilities are modular, because the Arduino Uno cannot handle many of them at one time.  In previous versions of Gizmo, I attempted to cram multiple simplified versions of these applications on the Uno: but nowadays Gizmo permits only a single application at a time on the Uno (see the file \texttt{All.h}).  The Drum Sequencer is too large to fit on the Uno at all, and the Step Sequencer can only fit in simplified form.  All of the applications combined fit just fine on the Mega (which you definitely ought to get instead of an Uno).
...
\item {\bf Drum Sequencer.}\quad The drum sequencer is similar to the step sequencer but has many features more oriented towards controlling drum synthesizers in the vein of a drum machine, rather than playing notes.  Whereas the step sequencer repeats a single 16-to-64-step sequence, the drum sequencer is has up to 15 such sequences (called {\it groups}) and can chain them together in a series of up to 20 transitions from group to group in order to define drum patterns for an entire song.  Each group can have from 1 to 64 steps, and up to 20 tracks, depending on the organization (there are 16 organization choices). Each track plays a single drum note on a single MIDI channel at a given velocity; the steps in the track simply state when the note should be played.  Like the step sequencer, you can specify a mute pattern for each track (and for each group). You have many speed options, including independent speed of groups, plus swing and other features.  There's an edit mode which allows you to toggle steps x0x-style, and another edit mode which lets you enter notes in real time as the sequencer is playing.  There's also a ``performance mode'' which allows chaining, playing along, etc., plus an additional ``group mode'' for changing and editing groups.  You can stipulate whether starting/stopping the sequencer will also start/stop the MIDI clock.    The drum sequencer will only fit on the Mega, and can store up to nine sequences.
...
You can in fact use four 8x8 backpacks as a square, but I would not recommend it: too many calls to I2C.   

{\color{red}\bf Warning:} LED screens draw significant current. Doubling the screens will double the current draw on the Arduino's regulator.  According to Adafruit, each 8x8 screen draws 160mA maximum at full brightness with all LEDs lit: thus a vertical stack would draw at most 640mA.  The regulator maxes out at 800mA, so you may want to run the screens at a lower brightness. I've not had any problems (and the regulator does not get warm) running two screens for long periods of time with my customary brightness of 4 on a Mega.


\paragraph{Customizing the Screen I2C Address}  Perhaps you might need to change the I2C addresses used to control your backpacks.  In this case, the four I2C addresses potentially used by the screen LEDs are found in the Gizmo file \texttt{LEDDisplay.h}.  They are by default (change them as you see fit):
...
\item {\bf \st{Modify your Arduino Software}}\quad It used to be that you had to perform surgery on your Arduino software to modify the Wire library.  This is no longer required: Gizmo now has its own copies of modified versions of the Wire library.
...
\item {\bf Do a Full Reset of Gizmo}\quad Power up the Arduino.  Then hold down all three pushbuttons on the MIDI Shield.  While doing so, press and release the reset button on the MIDI Shield.  Continue to hold down the three push buttons until the two LEDs on the MIDI Shield light up.\footnote{Sometimes you have to do this a couple of times on the Mega to get the lights to light up.}  Let go of the pushbuttons.  This procedure initializes the memory, options, and all files in the device.  See Section~\ref{initializinggizmo} for more details about the full reset and other, gentler, reset options.  
...
\paragraph{Arming the Clock}

In the Step Sequencer, Drum Sequencer, and Recorder, the clock is started as soon as you start playing.  But this doesn't make much sense for the Arpeggiator.  Instead, you have the Menu option of {\it arming the clock} so that it is starts when you enter the first note in your arpeggio. Or you can {\it disarm} the clock so that it doesn't start at all.  
...
When Gizmo has two vertical stacked screens, the Arpeggiator will take advantage of them to display a user arpeggio at full width.
...
\noindent The Step Sequencer makes it easy to create multitrack loops of notes and other MIDI data.  You can have twelve 16-note tracks, eight 24-note tracks, six 32-note tracks, four 48-note tracks, three 64-note tracks, or two 96-note tracks.  You can later reduce the length of these tracks to arbitrary values.
...
\paragraph{Chords {\it (Advanced Step Sequencer Only)}} You can have a track play chords instead of notes: just choose {\bf Chord (Track)} (see {\bf Menu Options} below) and select a chord.  Now the note you enter forms the root of that chord.  Note that you can't have a track fader when you have selected chords for a track.
...
\item The Patterns menu has been replaced with a menu called {\bf Repeats}, which indicates how a track is played in Performance mode.  Here you can set up a track to {\tt LOOP} forever, or to play 1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 16, 24, 32, or 64 times, or you can mark the track as the {\tt END}.  This doesn't affect Edit or Play Position mode.
\item Specifically, in Performance Mode the first track will play its {\it repeats} number of times: then the second track will play its (possibly different) {\it repeats} number of times, and so on, until we reach an {\tt END} track or we finish all the tracks. At that point will the sequence terminate, loop back to the beginning, or go to a new sequence as usual.
...
\begin{itemize} 
\item In all modes, {\it pairs} of tracks are played rather than single tracks.  You can have 6 pairs (totaling 12 tracks) of 16 steps each, 4 pairs (8 tracks) of 24 steps, 3 pairs (6 tracks) of 32 steps, or 2 pairs (4 tracks) of 64 steps.
\item The {\bf Repeats} menu sets values for {\it pairs} of tracks.  
\item When in Performance Mode, the first track {\it pair} will play its {\it repeats} number of times: then the second track {\it pair} will play is (possibly different) {\it repeats} number of times, and so on.
\item When in Play Position Mode, pressing the middle button will {\bf jump to the next track pair} to start playing (rather than mute).
...
Although it sounds odd, you can set up tracks in Monophonic and Duophonic formats to use chords.
...
\item {\bf Chord (Track)  {\it (Advanced Step Sequencer Only)}}\quad This lets you indicate that a track plays chords rather than single notes.  If you're using chords, you can't use the Fader (they use the same memory).  The chord options are:\footnote{If you'd like to customize the chords, they're located in the array {\tt const int8\_t PROGMEM CHORDS[24][4]}, and their names are in the array {\tt const char* chordMenuItems[25]} in the function {\tt void stateStepSequencerChord()} immediately afterwards, all in the file {\tt StepSequencer.cpp}.  Stick with 24 chords due to internal menu length restrictions.  Note that {\tt chordMenuItems} is 25 long because it also includes ``none'' at the beginning.}

\begin{center}
\begin{tabular}{@{}rl@{}c@{\ \ \ \ }rl@{}c@{\ \ \ \ }rl@{}c@{\ \ \ \ }rl@{}c@{\ \ \ \ }rl@{}}
\multicolumn{2}{c}{\it None}&&\multicolumn{2}{c}{\it Intervals}&&\multicolumn{2}{c}{\it Large Intervals}&&\multicolumn{2}{c}{\it Triads}&&\multicolumn{2}{c}{\it Triad + 7ths}\\
\cline{1-2}\cline{4-5}\cline{7-8}\cline{10-11}\cline{13-14}
\\[-0.9em]
\tt ----&None&&\tt MN3&min 3&&\tt O&Octave&&\tt MINOR&minor&&\tt 7&Dom 7\\
\tt &&&\tt MJ3&Maj 3&&\tt O-3&Oct + min 3&&\tt 1MINOR&min 1st Inv&&\tt 7MINOR&min 7\\
\tt &&&\tt 4&4&&\tt O+3&Oct + Maj 3&&\tt 2MINOR&min 2nd Inv&&\tt 7MAJOR&Maj 7\\
\tt &&&\tt 5&5&&\tt O+5&Oct + 5&&\tt MAJOR&Major&&\tt 7DIMINISHED&dim 7\\
\tt &&&\tt MN6&min 6&&\tt 2 O&2 Oct&&\tt 1MAJOR&Maj 1st Inv&&\tt &\\
\tt &&&\tt MJ6&Maj 6&&\tt -O&1 Oct Below&&\tt 2MAJOR&Maj 2nd Inv&&\tt &\\
\tt &&&\tt MN7&min 7&&\tt -2 O&2 Oct Below&&\tt &&&\tt &\\
\end{tabular}
\end{center}

\item {\bf Fader (Track)}\quad This lets you specify a volume modifier which is multiplied against the velocities of the notes in the track.  Possible values are 0 ... 15 as follows: 0 is fully hushed, 1 is 1/8 velocity, 2 is 1/4 velocity .... 4 is 1/8 velocity ... 8 is normal velocity ... 15 is almost twice velocity.  {The Fader is not available when using chords}, sorry (they use the same memory).
...
\begin{itemize}
	...
	\item {\texttt{TRAN}} \quad When you press a key, this transposes sequenced notes (on those tracks which have been set to be transposable). The key is not played.
	\item {\texttt{TRNP}}  {\it (Advanced Step Sequencer Only)} \quad Like \texttt{TRAN}, except the key is played as well (transpose with pass-through).
\end{itemize}
...
118	&	Any		& Swap\\
119	&	Any		& Pitch Shift\\
120	&	Any		& Set Chord\hspace{1.75in}{\it Note: change the chord with left pot, CC 14 / NRPN 0}\\
...
31	&	0--127		& Save (Select to set, Back to cancel)\\
\end{tabular}
}
...
\noindent {\bf Note: }{\it \ The Drum Sequencer is too big for the Arduino Uno.}\vspace{1em}
...
\item The Drum Sequencer has sufficient memory for these things because it doesn't store nearly the amount of per-step information the Step Sequencer does. The Step Sequencer stores in its steps {\it notes} with pitches and velocities; or {\it control changes} like CC or NRPN.  But in the Drum Sequencer each track is tied to a single {\bf drum note} played at a certain velocity on a certain MIDI channel.  Thus each step in the track is just an ``on'' or ``off'', indicating whether the drum should be triggered or not.
...
\item Each row in each group can have its own four-bar {\bf pattern} in the Step Sequencer sense of the term.
...
\begin{wrapfigure}{r}{1.8in}
\vspace{-1em}
\includegraphics[width=1.8in]{22val.pdf}
\vspace{-1em}\caption{\small Track and Group / Transition Numbers (Tracks and Transitions can go to 22; Groups go to 15)}
\vspace{-2em}
\label{trackgrouptransitionnumber}
\end{wrapfigure}

\vspace{1em}Each option is displayed on-screen as {\it steps/groups/tracks}.  Why these choices in particular? Because they squeeze the most out of Gizmo's very limited memory.   

After you have chosen a format, you will be asked to provide one more piece of information: a {\bf base drum note}.  Drum synthesizers typically have all their notes laid out in order, starting with one lowest note.  For example, the Novation Drumstation places its TR-808 Kick at C0 (MIDI note key 24), then the TR-808 Rimshot at C\#0, then the TR-808 Snare at D0, and so on.  If you give C0 as your base note, then Gizmo will set the drum note pitch of track 1 to C0, the pitch of track 2 to C\#0, the pitch of track 3 to D0, and so on, and so will set up each track to play a different drum. This can help greatly in laying out the drums for your sequence.  (You can of course modify all of this afterwards).
...
\paragraph{Velocity Curves}  The Drum Sequencer's velocities are the numbers 1...8.  How do these translate to MIDI velocities 0...127?  There are two options.  We can either interpret 1...8 as {\it linear} or as {\it exponentially increasing} values: which you prefer would depend on what fits better with the velocity curve of your drum synthesizer.  The default is a linear curve, but you may prefer an exponential curve if it makes accents more consistent. 
...
Notice that with the exponential curve, increasing (accenting) by +1 changes the MIDI velocity, and perhaps the underlying volume, by about +25\%; increasing by +2 changes by +60\%; increasing by +3 changes by +100\%; increasing by +4 changes by +150\%; and so on.  That's convenient.  But your drum machine might {\it already} have an exponential curve built in, and so then you'd want the linear velocity curve.  You can switch between curves with a menu option.


\paragraph{Accent and Ghost Tracks}  The Drum Sequencer can only have one note velocity per track.  To play accents (louder beats than normal) or ghost notes (softer beats than normal), you need to set up another track to do the accent/ghost notes for a given track, or you can have an 808-style {\bf global accent track}.  The former is more flexible, but the latter is easier and doesn't eat up as many tracks.
...
Let's start with the first choice.  To add accents to a given track, just set up a second track\,---\,often the very next track\,---\,with the same drum note and MIDI channel, but a different velocity, and play accented beats there.  There's a helpful menu option called {\bf Accent Track}, on page~\pageref*{accenttrack}, which does exactly this, increasing the accent velocity by 2.  Similarly the menu option {\bf Ghost Track} will reduce the velocity by 2.  There's a little trick to make entering accents more convenient: if you hold down a low key or high key, as shown in Figure~\ref{editmodekeys}, while playing another note, then that note will instead be entered in the next track (notionally your accent track), not the current track. 
...
\paragraph{Choke Tracks}

With some care, you can set up a track to {\bf choke} another track, or to choke itself whenever a note is played in the track.  To choke a track means to hush all currently playing sounds in the track.  This is often done with crash cymbals, for example.  

Due to memory restrictions, choking is done in an odd way.  To set up track B to choke track A, track B must be {\it immediately below} track A.  Then set the MIDI Channel (the \texttt{TRACK}\(\rightarrow\){\texttt{OUT} menu) of track B to \texttt{CHOKE}.   B will use the same MIDI Channel as A, and will choke it.  If you want multiple tracks (B and C say) to choke A, have them both directly below A.  Then set the MIDI Channel of {\it both} B and C to  \texttt{CHOKE}.  Muting or (via Patterns) hushing Track B disables its choking, so to set up track B to {\it only} choke A, but play no notes itself, set Track B's drum note to something out of the drum synth's range.  Finally, if you'd like a track B to choke {\it itself} prior to playing a note, set it below some other track A, and then set B's MIDI Channel to  \texttt{SELF CHOKE}.  Track B will use the same MIDI Channel as A, but will choke itself.\footnote{Yes, I know this is weird\,---\,it's the only way I could fit it in with memory restrictions.} 

Some notes.  First, keep in mind that if you move a choke track, or the track it's choking, it's now choking a different track.  Likewise a self-choke track would now be using the MIDI Channel of the new track above it.  This can be confusing if you're not careful.  Also you can't distribute the track info of a choke or self-choke track.  Second note that the first track cannot be a choke or self-choke track: it's not below anything.  If you copy or swap a choke/self-choke track to the first position, it will automatically be set to MIDI Channel OFF.  
...
{\tt LOOP} indicates that the group should repeat forever, never transitioning, unless you manually schedule a transition for it (you can do this in Performance Mode).  We'll get to {\tt BIG LOOP} in a moment.  
...
\item {\bf Randomize}\quad Specifies the probability (1--100) that when you clear an already empty track, a given step will instead be randomly turned on.  If \texttt{- - - -}  then the track is  left empty (the default).
...
\item {\bf Rotate Track}\quad Shifts and rotates the local track notes so that the note at the mark is move to the current position.  The mark is then cleared.  If the mark and current position are the same, or are on different tracks, or the mark is not yet set, or the mark is in another group, \texttt{CANT} is displayed.
\end{itemize}
...
You can also choose \texttt{CHOKE} or \texttt{SELF CHOKE}, which sets the track to be a choke or self-choke track respectively: it will use the MIDI Channel of the track immediately above it.  The first track cannot be a choke or self-choke track.
...
\item {\bf Rotate Group}\quad Shifts and rotates all notes in the group so that the note at the mark is move to the current position.  The mark is then cleared.  If the mark and current position are the same, or the mark is not yet set, or the mark is in another group, \texttt{CANT} is displayed.
\end{itemize}
...
\item[]~\vspace{-2em}
\begin{itemize}
	\item {\bf Edit}\quad Edits the selected transition.  You will be first asked to pick a group for the transition (you'll see a \texttt{G}), or \texttt{DFLT} to select the current group.  You can also select \texttt{- - - -} to indicate extra options.  
	
	If you selected a group or \texttt{DFLT}, you will then be asked to specify the number of repeated iterations for the group (you'll see an \texttt{R}).  Your choices are \texttt{LOOP}, 1, 2, 3, 4, 5, 6, 7, 8, 9, 12, 16, 24, 32, 64, or \texttt{BIG LOOP}.  
	
	If you instead selected \texttt{- - - -}, you'll be then asked to select among extra options.  These options are: \texttt{END}, R2-1, R2-2, R2-3, R2-4, R2-L, R3-1, R3-2, R3-3, R3-4, R3-L, R4-1, R4-2, R4-3, R4-4, and R4-L.  The various {\it R-} options indicate loops with random group selection. They are:\\
	\vspace{1em}
...
~	%% This fixes a weird bug: if I don't have this (which is essentially its own blank paragraph) then the next paragraph is misformatted 
...
%\item {\bf Show Note}  (or {\bf No Show Note})\quad  Toggles whether the upper of two vertically stacked 16x8 LED screens is dedicated to displaying the value of the note at the cursor; or if its is just used for extra track display space.  In the former case, notes are shown with the note pitch/octave and velocity (1...8).  This menu has no effect at all if you only have one LED screen.
...
When Gizmo has two vertical stacked screens, the Drum Sequencer will take advantage of them to display the entire sequence.  You now have 14 rows to display tracks: since you can have up to 20 tracks, you may still have to scroll.  Additionally, when selecting a group, the upper LEDs will display the first 7 rows of the sequence.
...
\hline\\[-0.5em]
 ...
 91	&	Any		& Next Transition&{\it Only separate from Next Sequence (120) via CC}\\
 92	&	Any		& Toggle Click Track\\
 93	&	Any		& Set Mark\\
 94	&	Any		& Copy Track in Group\\
 95	&	Any		& Swap Tracks in Group\\
...
\hspace{\fill}\smash{\\\includegraphics[width=1in]{note.pdf}}\vspace{-1.5em}
	...
	\item{\bf  Menu Delay} (Mega Only)\\
		Gizmo often will display text (such as \texttt{SWING}) by filling the screen with the first few letters (in this case, \texttt{SWI}), then {\it delaying} for a certain interval, then scrolling through the whole text horizontally. You can specify how long that delay is.
		...
		\begin{description}
			...
			1/4\>Seconds\\
			1/3\>Seconds \>(actually 3/8 seconds)\hspace{1in}\=\smash{\includegraphics[width=2in]{fractions.pdf}}\\
			1/2\>Seconds\\
			1\>Second\>(default)\\
			2\>Seconds\\
			3\>Seconds\\
...
%		Toggles whether Gizmo will send control voltage and gate information.  If {\bf CV+Velocity}, then control voltage will be sent out DAC A (I2C address 0x62), velocity will be sent out DAC B (I2C address 0x63), and digital pin 8 will be set HIGH (5V) when a note is played (or LOW (0V) when a note is released), thus acting as a Gate.\footnote{As mentioned in an earlier footnote, Gizmo assumes you're DACs of this kind: https:/\!/www.adafruit.com/product/935} If {\bf CV+Aftertouch}, then the same is done except that aftertouch will be sent out DAC B instead of velocity.  If {\bf No CV}, then nothing is sent.  Pitch Bend at present has no effect.	\footnote{Note that at present if you have set either knob in the Controller to control voltage, that DAC will be unavailable even outside the Controller.  I'll fix that soon.}
	...
	\item{\bf The About Screen}\\
		Presently says:\quad \texttt{GIZMO V10 (C) 2021 BY SEAN LUKE}
	\end{description}
...
\begin{wrapfigure}{r}{2in}
\includegraphics[width=2in]{MultiGizmoLayout.pdf}
\vspace{-1em}\caption{\small Example Layout}\vspace{-2em}
\label{MultiGizmoLayout}
\end{wrapfigure}
...
\paragraph{Handling the Clock} You will often be turning on bypass in your upstream Gizmo in order to control a downstream Gizmo or to enter note data into it.  If this upstream Gizmo is handling the clock, turning on bypass will stop the clock, which you probably don't want.  Instead, you can set the clock to {\bf Merge}, and it will continue to run even in bypass mode.
...
\paragraph{Example} Figure~\ref{MultiGizmoLayout} shows a real layout I've used: you get bonus points for identifying the devices in question.   In this layout, both Gizmo A and B are sent note information by the Keyboard controller and are controlled by  the CC controller.  The keyboard controller is fixed to channel 1, but Gizmo A can always convert channel 1 to any other channel via the MIDI Routing Channel to play a downstream synthesizer (see ``Secrets of the Top Level Menu'', Section~\ref{secretsofthetoplevelmenu}).

Gizmo A is set up with MIDI In = Channel 1 (same as Keyboard controller), MIDI Out = 10 (same as drum synthesizer), MIDI Control = Channel 16 (same as CC controller), and Clock = Merge, so clock information is sent even if bypass is turned on.  Gizmo A is playing the Drum Sequencer.

Gizmo B is set up with MIDI In = Channel 1, MIDI Out = 1 (same as synthesizer 1), MIDI Control = Channel 16, and Clock = Use (Consume is fine too), so it uses the clock information sent by Gizmo A.  Gizmo B is playing the Step Sequencer, with some tracks set to Channel 2 (same as synthesizer 2).   

To control Gizmo B, you just need to turn on bypass for Gizmo A, which passes through to B both keyboard and CC controller information.  When starting the Drum Sequencer on Gizmo A, the clock is also started, which does the same on Gizmo B.  If you want to play just Gizmo B, you can always set its Clock to Ignore.
