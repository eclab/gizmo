...
\item {\bf Chord (Track)  {\it (Advanced Step Sequencer Only)}}\quad This lets you indicate that a track plays chords rather than single notes.  The chord options are: 

\begin{itemize}
\item None (\texttt{----})
\item {\it Intervals:} minor 3, Major 3, 4th, 5th, minor sixth, Major sixth, octave, octave + minor 3, octave + Major 3, octave + fifth, 2 octaves, 1 octave {\it below}, 2 octaves {\it below}
\item {\it Triads:} minor, minor 1st inversion, minor 2nd inversion, Major, Major 1st inversion, Major 2nd inversion
\item {\it Sevenths:} dominant, Major, minor, fully diminished
\end{itemize}

If you'd like to customize the chords, they're located in the array {\tt const int8\_t PROGMEM CHORDS[24][4]}, and their names are in the menu string array immediately afterwards, in the file {\tt StepSequencer.cpp}.


\item {\bf Fader (Track)}\quad This lets you specify a volume modifier which is multiplied against the velocities of the notes in the track.  Possible values are 0 ... 15 as follows: 0 is fully hushed, 1 is 1/8 velocity, 2 is 1/4 velocity .... 4 is 1/8 velocity ... 8 is normal velocity ... 15 is almost twice velocity.  {\it The Fader is not available when using chords}, sorry (they use the same memory).
