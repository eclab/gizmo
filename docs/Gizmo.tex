...
\item {\bf MIDI Control Surface}.  Gizmo has two potentiometers and two buttons.  Each can be configured to send a unique Program Change, Control Change, NRPN, or RPN commands, or (on the Mega) output voltage values via either of two DACs.  The potentiometers can only send MSB values.

\item {\bf \textit{Mega Only:}\quad Keyboard Splitting}  You can split incoming MIDI notes by pitch and route them to different MIDI out channels.
\end{itemize}
...
\item {\bf Screen Brightness and Menu Delay}  You can change the screen brightness.  
...
When you are satisfied with your arpeggio, Long-Press the Select button to save.  You'll be asked for a slot to save in, and after that, you'll be back in the main Arpeggiator menu.
...
You enter notes by moving the edit cursor to the a location, then pressing keys.  You can also press the Middle Button to enter a rest, or Long Press the Middle Button to enter a {\it continuation}.  A continuation just means ``keep playing the previous note'': it essentially lengthens the previous note.  You can enter multiple continuations in series to make a note as long as you like.
...
\item {\bf Velocity}\quad This lets you specify a velocity that {\it all} notes in the track will use (as a value 0...127).  If you would instead prefer that each note in the track use its own independent velocity as you had entered them, you can choose ``DFLT''\footnote{Yes, I realize that ``default'' isn't the best term here.}
...
The Recorder records nothing but MIDI NOTE ON and NOTE OFF messages.  It presently quantizes these messages, rounding them up or down to the nearest MIDI Clock pulse, that is, 1/24 of a quarter note.
...
\vspace{-2em}\caption{\small Octave Values}\vspace{-6em}
\label{octave}
\end{wrapfigure}
...
\hspace{\fill}\smash{\\\includegraphics[width=1in]{note.pdf}}\vspace{-1.5em}
	...
	\item{\bf \textit{Mega Only:}\quad Voltage} (or {\bf No Voltage})\\
		Toggles whether Gizmo will send voltage information out via the optional DACs in response to notes played or to Controller potentiometers being turned.  Note that Voltage is sent in response to an I2C signal, and this may cause a very slight MIDI delay.
